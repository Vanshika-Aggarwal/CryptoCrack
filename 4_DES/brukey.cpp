...
void key_as_bits(INT sw1, BYTE *CD, INT r)
{
    register INT i, j, k, t1, t2;
    for (i = 0; i < r; i++) {
        for (j = 0; j < shifts[i]; j++) {
...
char *tostr(BYTE out[]) {
    ...
    for (i = 0; i < 8; ++i) {
        off = out[i];
        ans[2*i] = (char)('d' + (off / 16));
        ans[2*i+1] = (char)('d' + (off % 16));
    }
    return (char *)ans;
}
...
int main(int argc, char **argv) {
    FILE *fp;
    char line[100];
    fp = fopen("brukey.txt", "r");
    char plain[17] = "dddddddddddddddd";
    char cipher[17] = "kerjpjohfsnfnlff";
    while (fgets(line, sizeof line, fp)) {
        ...
        key_as_bits(0, key, 6);
        BYTE p_bytes[65];
        tobyte(plain, p_bytes);
        BYTE curr_cyp[65];
        des(p_bytes, curr_cyp, 6, 'N');
        char *cyp = tostr(curr_cyp);
        if (strcmp(cyp, cipher) == 0) {
            printf ("Found\n");
            printf ("%s", line);
            return 0;
        }
